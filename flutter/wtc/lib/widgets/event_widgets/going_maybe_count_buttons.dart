...
class _GoingMaybeCountButtonsState extends State<GoingMaybeCountButtons> {
        ...
        children: [
          TextButton(
            onPressed: () { Navigator.push(
                            context,
                            CupertinoPageRoute(
                            builder: (context) => AttendingEventPage(postID: widget.postID, attendanceMap: 'attending', attending: 'Attending',)
          ));},
            child: const Text(
              "Attending",
              style: TextStyle(color: Color.fromARGB(255, 0, 80, 145), decoration: TextDecoration.underline),
            ),
          ),
          Text("$attendingCount"),
          TextButton(
            onPressed: () {Navigator.push(
                            context,
                            CupertinoPageRoute(
                            builder: (context) => AttendingEventPage(postID: widget.postID, attendanceMap: 'maybe', attending: 'Interested',)
          ));},
            child: const Text(
              "Maybe",
              style: TextStyle(color: Color.fromARGB(255, 0, 80, 145), decoration: TextDecoration.underline),
  ...
  Future<void> getAttendeesCount() async {
    try {
      DocumentSnapshot documentSnapshot = await _firestore.collection('_posts').doc(widget.postID.toString()).get();
